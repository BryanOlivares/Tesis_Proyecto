...
////////////////////////
    ...
    public static function apply_coupon_to_the_order($order_id, $coupon_num){
        ...
        /// get coupon amount
        $coupon_amount_arr = self::get_coupon_amount($coupon->ID, true);
        $coupon_amount = $coupon_amount_arr["value"];

        if ($coupon_amount_arr["type"] === 'percent') {
            $coupon_amount = BABE_Order::get_order_total_amount($order_id) *  ( $coupon_amount / 100 );
        }
...
//////////////////////////////////////                       
    ...
    public static function get_coupon_amount( $coupon_id, $get_type='') {
	    $percent  = (float)get_post_meta($coupon_id, '_coupon_percent', 1);
	    if ($percent){
	    	$return = array( 'value' => $percent, 'type'=> 'percent');
	    } else {
		    $amount = (float)get_post_meta($coupon_id, '_coupon_amount', 1);
