...
////////////////////////
    ...
    public static function sync_two_calendars( $post_id_from, $post_id_to ){
        ...
        $sql = "SELECT date_from, guests, in_schedule
        ...
        if ( !empty($added) ){
            ...
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                "INSERT INTO ".self::$table_av_cal." (booking_obj_id, date_from, guests, in_schedule) VALUES ".implode( ', ', $new_values )
            );
        }
     ...
     */
     public static function get_av_cal($booking_obj_id, $date_from = '', $date_to = '', $order_item_args = [], $ignore_stop_booking = false){
        ...
        $output = [];
         ...
         $stop_booking_before = get_post_meta( $booking_obj_id, 'stop_booking_before_'.$category_slug, 1 );

         if ( !$stop_booking_before && $stop_booking_before !== '0' && $stop_booking_before !== 0 ){

             $stop_booking_before = $rules_cat['rules']['stop_booking_before'] ? $rules_cat['rules']['stop_booking_before'] : 0;
         }
        ...
        // normalize dates
         ...
         $request_with_dates = $date_from && $date_to;
        ...
        //// check stop booking before ... hours //////////
         ...
         if ( !$ignore_stop_booking ){
             ...
             if ($today_obj >= $date_from_obj && $rules_cat['rules']['basic_booking_period'] === 'recurrent_custom' ) {
        ...
        if ( empty($av_cal) ){
         ...
         foreach($av_cal as $cal_row){
             ...
             if ( !$cal_row['in_schedule'] ){
                 ...
                 $max_guests_add
                 && $curren_date_obj >= $date_test_from_obj
                 && $curren_date_obj <= $date_test_to_obj
             ){
                 $cal_row_av_guests += $max_guests_add;
             }

             if ( !$ignore_stop_booking && $today_obj > $curren_date_obj ) {
                 $cal_row_av_guests = 0;
             }
