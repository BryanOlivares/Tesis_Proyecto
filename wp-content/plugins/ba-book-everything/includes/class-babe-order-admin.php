...
////////////////////////
    ...
    if ( is_admin() && $pagenow=='edit.php' && $_GET['post_type']==BABE_Post_types::$order_post_type && $_GET['s'] != '') {
        $join .='LEFT JOIN '.$wpdb->postmeta. ' postmetasc ON '. $wpdb->posts . '.ID = postmetasc.post_id LEFT JOIN '.BABE_Order::$table_order_items. ' ON '. $wpdb->posts . '.ID = ' . BABE_Order::$table_order_items . '.order_id LEFT JOIN '.BABE_Order::$table_order_itemmeta. ' ON '. BABE_Order::$table_order_items . '.order_item_id = ' . BABE_Order::$table_order_itemmeta . '.order_item_id ';
     }
    }
    return $join;
}
...
////////////////////////
    ...
    if ( is_admin() && $pagenow=='edit.php' && $_GET['post_type']==BABE_Post_types::$order_post_type && $_GET['s'] != '') {
        ...
        $new_where = "(".$wpdb->posts.".post_title LIKE $1) OR ((postmetasc.meta_key = '_status') AND (postmetasc.meta_value LIKE $1)) OR ((postmetasc.meta_key = 'first_name') AND (postmetasc.meta_value LIKE $1)) OR ((postmetasc.meta_key = 'last_name') AND (postmetasc.meta_value LIKE $1)) OR ((postmetasc.meta_key = 'email') AND (postmetasc.meta_value LIKE $1)) OR ((postmetasc.meta_key = 'phone') AND (postmetasc.meta_value LIKE $1)) OR (".BABE_Order::$table_order_items.".order_item_name LIKE $1) OR ((".BABE_Order::$table_order_itemmeta.".meta_key = 'date_from') AND (".BABE_Order::$table_order_itemmeta.".meta_value LIKE $1))";
