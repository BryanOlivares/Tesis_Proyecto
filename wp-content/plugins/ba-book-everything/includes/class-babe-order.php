...
////////////////////////
    ...
    public static function get_order_payment_gateway_fee_percents($order_id ){
        ...
        return (float)get_post_meta($order_id, '_payment_gateway_fee_percents', 1);
    }
...
////////////////////////
    ...
    public static function get_order_payment_gateway_fee_title( $order_id ){
        ...
        return get_post_meta($order_id, '_payment_gateway_fee_title', 1);
    }
...
//////////////////////
    ...
    public static function wp_update_order_meta($meta_id, $post_id, $meta_key, $meta_value) {
        ...
        $action_new_order = !get_post_meta($post_id, '_email_new_order', 1) ? 'new_order' : 'order_updated';

        // old_status => new_status => notification_type
        $status_router = [
            'draft' => [
                'av_confirmation' => 'new_order_av_confirm',
                'payment_received' => 'new_order',
                'payment_expected' => 'new_order_to_pay',
                'payment_deferred' => 'new_order',
                'completed' => 'new_order',
            ],
            'av_confirmation' => [
                'not_available' => 'order_rejected',
                'canceled' => 'order_canceled',
                'payment_received' => $action_new_order,
                'payment_expected' => 'new_order_to_pay',
                'payment_deferred' => $action_new_order,
                'completed' => $action_new_order,
            ],
            'payment_deferred' => [
                'not_available' => 'order_canceled',
                'canceled' => 'order_canceled',
                'payment_received' => $action_new_order,
                'payment_expected' => 'new_order_to_pay',
                'completed' => $action_new_order,
        ...
        $status_router = apply_filters('babe_order_status_update_status_router', $status_router, $post_id, $old_status, $meta_value);

        $action = !empty( $status_router[$old_status][$meta_value] ) && is_string($status_router[$old_status][$meta_value]) ? $status_router[$old_status][$meta_value] : '';
        ...
        switch ($action){
            ...
            case 'order_updated':
                BABE_Emails::send_admin_email_order_updated($post_id);
                BABE_Emails::send_email_order_updated($post_id);
                break;
            ...
            case 'new_order_to_pay':
                ...
                if ( $action_new_order === 'new_order' ){
                    BABE_Emails::send_admin_email_new_order($post_id);
                }
                break;

            case 'order_canceled':
                BABE_Emails::send_admin_email_order_canceled($post_id);
                BABE_Emails::send_email_order_canceled($post_id);
...
//////////////////////////////
    ...
    public static function sanitize_booking_vars($arr){
        ...
        if ( !empty($mandatory_services) ){
            $output['services'] = array_merge($output['services'], array_map('absint', $mandatory_services));
        }
            ...
            if ( !empty($av_cal) && $av_cal[$date_from_obj->format('Y-m-d')]['start_day']){
                ...
                if ($rules_cat['rules']['basic_booking_period'] == 'day' || $rules_cat['rules']['basic_booking_period'] == 'night'){
                    if ( $rules_cat['rules']['basic_booking_period'] == 'night' ){
                        // for number of nights calculation purpose
        ...
        if (isset($_POST['booking_obj_id']) && BABE_Post_types::is_post_booking_obj($_POST['booking_obj_id']) && isset($_POST['action']) && $_POST['action'] == 'to_checkout'){
            ...
            if (!empty($post_arr['guests']) && $post_arr['booking_obj_id'] && $post_arr['date_from'] && $post_arr['date_to']){
                ...
                if ($av_guests){
                    ...
                    do_action('babe_checkout_payment_gateway_selected', $order_id, '');
