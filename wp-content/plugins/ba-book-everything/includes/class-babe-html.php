    	...
    	if( empty($args['order_id']) ){
         ...
         foreach ($order_items_arr as $order){
             ...
             for ($i = 2; $i <= $total_number_of_guests; $i++){
                 $output .= '<h4>'. sprintf(__( 'Extra guest %s', 'ba-book-everything' ), $i) .'</h4>';
    ...
    public static function ajax_booking_calculate() {
        ...
        if (isset($_POST['post_id']) && BABE_Post_types::is_post_booking_obj($_POST['post_id']) && isset($_POST['date_from']) && isset($_POST['date_to']) && isset($_POST['nonce']) && wp_verify_nonce( $_POST['nonce'], self::$nonce_title)){
          ...
          $services = isset($data_arr['booking_services']) && is_array($data_arr['booking_services']) ? array_map('absint', $data_arr['booking_services']) : [];
            $mandatory_services = BABE_Post_types::get_post_mandatory_service_ids($babe_post);
...
////////////////////////////
    ...
    public static function booking_form_av_times($post_id, $date_from, $order_item_args = array()){
        ...
        $guests_min = $av_cal[$date_from_obj->format('Y-m-d')]['min_booking_period'] ? $av_cal[$date_from_obj->format('Y-m-d')]['min_booking_period'] : 0;
						...
						<div class="select_guests_block input_select_field">
							<div class="input_select_title">
                                 ...
                                 <div class="input_select_wrapper">
                                    ...
                                    <ul class="input_select_list">
                                    '.self::get_range_input_select_options( ( $number_of_ages == 1 ? $guests_min : 0 ), $guests_max, 1, $selected_value).'
                                    </ul>
                                    <i class="fas fa-chevron-down"></i>
                                 </div>  
                            </div>
    ...
    public static function booking_form($booking_obj_id = 0){
        ...
        $input_fields['date_to'] = '';
            ...
            <div class="booking-date">
	            ...
	            $input_fields['date_to'] = '
            <div class="booking-form-block booking-date-block">
                <label class="booking_form_input_label"><span class="booking_form_step_num">'.$i.'</span>'.apply_filters('babe_booking_form_date_from_label', __('To:', 'ba-book-everything')).'</label>
            <div class="booking-date">
                   <i class="far fa-calendar-alt"></i>
				   <input type="text" class="booking_date" id="booking_date_to" name="booking_date_to" value="'.$date_to.'" placeholder="'.apply_filters('babe_booking_form_date_to_placeholder', '').'" data-post-id="'.$post_id.'" autocomplete="off">
			</div>
...
////////////////////////////
    ...
    public static function order_items($order_id){
        ...
        if ($order_id){
            ...
            foreach($order_items_arr as $item_id => $item){
                ...
                $guests_html = '
                  ...
                  foreach($item['meta']['guests'] as $age_id => $guests_num){
                     ...
                     $age_title = isset($ages_arr_ordered_by_id[$age_id]) ? $ages_arr_ordered_by_id[$age_id]['name'] : ( $guests_num > 1 ? __( 'persons', 'ba-book-everything' ) : __( 'person', 'ba-book-everything' ) );
                ...
                /// Total item price                
                $discount_note = $item['meta']['price_arr']['discount'] ? '<tr><td class="order_item_discount_note">'.sprintf(__( 'Price Cut %d%% off applied', 'ba-book-everything' ), $item['meta']['price_arr']['discount']).'</td></tr>' : '';
            ...
            <span class="order_items_row_fee_amount">'.BABE_Currency::get_currency_price($fee_amount).'</span>
            </td></tr>';
                    }
                }
            ...
            <td class="order_items_row_total_amount order_items_row_subtotal">
            <span class="order_items_row_gateway_fee_amount">'.BABE_Currency::get_currency_price($total_item_prices['total_payment_gateway_fee']).'</span>
            </td></tr>';
...
///////////////////////////////////////
    ...
    public static function ajax_checkout_payment_method_changed(){
        ...
        $output['amount_group'] = self::checkout_form_element_amount_group($total_amount, $prepaid_amount, $payment_model);
...
/////////////////////////////////////
     ...
     * @return string
     */
    public static function checkout_form_element_amount_group($total_amount, $prepaid_amount, $payment_model){

       return '<div class="amount_group"> 
                <label class="checkout_form_amount_label">'.__('Amount to Pay now:', 'ba-book-everything').'</label>
                <div class="checkout_form_pay_total">
                <input type="radio" name="payment[amount_to_pay]" id="order_amount_to_pay_deposit" value="deposit" checked="checked"><label for="order_amount_to_pay_deposit">'.BABE_Currency::get_currency_price($prepaid_amount).' '.($payment_model == 'deposit_full' ? __('(deposit)', 'ba-book-everything') : '').'</label>
                </div>
                '.($payment_model == 'deposit_full' ? '
                <div class="checkout_form_pay_total">
                <input type="radio" name="payment[amount_to_pay]" id="order_amount_to_pay_full" value="full"><label for="order_amount_to_pay_full">'.BABE_Currency::get_currency_price($total_amount).' '.__('(full)', 'ba-book-everything').'</label>
                </div>
                ' : '').'
            </div>';
   }
    ...
    public static function checkout_form($args){
        ...
        $input_fields = apply_filters('babe_checkout_input_fields_arr', $input_fields, $args);
            ...
            if($action == 'to_pay'){
                ...
                $payment_fields = self::checkout_form_element_amount_group($total_amount, $prepaid_amount, $payment_model);
                ...
                $payment_details_text = sprintf(__('You could pay after the items availability confirmation! <br> Confirmation will be sent to your e-mail. <br> Amount to pay online will be %s.', 'ba-book-everything'), BABE_Currency::get_currency_price($prepaid_amount));
                 ...
                 if ( !empty($rate_current) ){
                  ...
                  <div class="view-rate-details">
                    ...
                    if ( $rate_current['min_booking_period'] ){
                        $prices_table .= '
                        <span class="rate_details_label">'.__('Minimum booking:', 'ba-book-everything').'</span> <span class="rate_details_value">'.$rate_current['min_booking_period'].' '.$units.'</span>
                        ';
                    }
                    if ( $rate_current['max_booking_period'] ){
                        $prices_table .= '
                        <span class="rate_details_label">'.__('Maximum booking:', 'ba-book-everything').'</span> <span class="rate_details_value">'.$rate_current['max_booking_period'].' '.$units.'</span>
                        ';
                    }
                    $prices_table .= '
                    </div>';
    ...
    public static function list_fees( $post, $selected_arr = [] ){
        ...
        foreach($fees_arr as $fee){
            ...
            if ( $general_price ){
                ...
                $sr_block .= __( 'free', 'ba-book-everything' );
            }
        ...
        foreach($services_arr as $service){
            ...
            if ( !in_array($before_service_type, array('booking', 'day', 'night')) ){
                ...
                foreach($ages as $age){
                    ...
                    if (!$is_percent){
                        ...
                        if ($price_value !== ''){
                            ...
                            $price_value_html = $price_value == 0 ? __( 'included', 'ba-book-everything' ) : BABE_Currency::get_currency_price($price_value + round($price_value * $tax_am, 2));
                        ...
                        if ( !$is_other_prices ){
                            ...
                            $sr_block .= '<span class="service_price_line"><label>'.apply_filters('translate_text', $age['name']).':</label>'.$general_price.' '.__( '%/booking ', 'ba-book-everything' ).'</span>';

                        } elseif ( isset($other_prices[$age['age_id']]) && $other_prices[$age['age_id']] !== '' ){

                            $sr_block .= '<span class="service_price_line"><label>'.apply_filters('translate_text', $age['name']).':</label>'.$other_prices[$age['age_id']].' '.__( '%/booking ', 'ba-book-everything' ).'</span>';
                ...
                if (!$is_percent){
                    ...
                    $sr_block .= $general_price == 0 ? __( 'included', 'ba-book-everything' ) : BABE_Currency::get_currency_price($general_price + round($general_price * $tax_am, 2));
            ...
            if ( $service['is_mandatory'] ){
                ...
                $mandatory_fields[] = apply_filters('babe_list_add_services_block_html', $sr_block, $service, $ages);

            } else {

                $optional_fields[] = apply_filters('babe_list_add_services_block_html', $sr_block, $service, $ages);
            }
                    ...
                    if (!in_array($before_service_type, array('booking', 'day', 'night'))){
                     ...
                     foreach($ages as $age){
                        ...
                        if (!$is_percent){
                          ...
                          $price_value_html = $general_price == 0 ? __( 'included', 'ba-book-everything' ) : BABE_Currency::get_currency_price($general_price + round($general_price * $tax_am, 2));
