       ...
       function check_base_rate(){
		...
		success : function( msg ) {
            if (msg != ''){
                ...
                $('#publish').parent().find('#babe_post_publish_notice').first().remove();
            } else {
              $('#publish').css('display', 'none');
              if ( !$('#publish').parent().find('#babe_post_publish_notice').first().length ){
                  $('#publish').parent().append('<div id="babe_post_publish_notice" class="notice notice-error"><a href="#deposit_fixed"><p>'+babe_prices_lst.messages.at_least_one_rate_required+'</p></a></div>');
              }
            }    
		  }
        });
